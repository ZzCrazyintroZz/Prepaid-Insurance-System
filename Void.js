...
function voidPrepaid(params) {
  try {
    ...
    if (remaining <= 0) return { ok: false, error: 'Document ' + docNo + ' is fully amortized (remaining ' + remaining + ')' };
...
function batchVoidPrepaid(docList, voidDate, type, lossGL) {
  try {
    if (!docList || !Array.isArray(docList) || docList.length === 0) {
      return { ok: false, error: 'Please specify documents to void' };
    }
    type = type || 'refund';
    ...
    return {
      ...
      failed: failed,
      results: results,
      message: 'Batch Void: ' + succeeded + ' success, ' + failed + ' failed of ' + docList.length + ' items'
    };
  } catch (e) {
    return { ok: false, error: e.message };
  }
