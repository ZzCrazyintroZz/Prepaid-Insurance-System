...
function setBudgetData(period, items) {
  try {
    if (!period) return { ok: false, error: 'Please specify period' };
    ...
    return { ok: true, message: 'Budget saved for period ' + period + ' (' + cleanItems.length + ' items)' };
  } catch (e) {
...
function getDashboardData() {
  try {
    ...
    var cached = chunkedCacheGet_(cacheKey);
    if (cached && cached.trend) return cached;
    ...
    var expiring30Count = 0;
    var expiring60Count = 0;
    var expiring90Count = 0;
    var expiring90plusCount = 0;
    var maxAmount = 0;
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (datesValid) {
        ...
        // --- Expiry buckets (30/60/90/90+) ---
        var daysToEnd = Math.round((end - now) / (1000 * 60 * 60 * 24));
        if (daysToEnd >= 0) {
          if (daysToEnd <= 30) expiring30Count++;
          else if (daysToEnd <= 60) expiring60Count++;
          else if (daysToEnd <= 90) expiring90Count++;
          else expiring90plusCount++;
        }
      }
    ...
    var result = {
      ...
      expiring60: expiring60Count,
      expiring90: expiring90Count,
      expiring90plus: expiring90plusCount,
      momChange: momChange,
    ...
    // Cache 5 min with chunking (supports >100KB)
    try { chunkedCachePut_(cacheKey, result, 300); } catch(e) {}
