...
function readInputData_() {
  ...
  try {
    var cachedStr = CacheService.getScriptCache().get('input_data_v2');
    if (cachedStr) {
      var parsed = JSON.parse(cachedStr);
      if (parsed && Array.isArray(parsed) && parsed.length > 0) {
        _inputDataCache = parsed;
  ...
  _inputDataCacheTime = now;
  try {
    CacheService.getScriptCache().put('input_data_v2', JSON.stringify(rows), 300); // 5 min
  } catch(e) {}
  return rows;
}
...
function deletePrepaidRecord(docNo) {
  try {
    ...
    // Log the action
    logAction('CRUD', 'ลบเอกสาร: ' + docNo + (hasBeenAmortized ? ' (มีรายการตัดจ่ายแล้ว)' : ''));
    ...
    return {
      ok: true,
      message: 'ลบเอกสาร ' + docNo + ' เรียบร้อย' + (hasBeenAmortized ? ' (หมายเหตุ: เอกสารนี้มีรายการตัดจ่ายแล้ว)' : ''),
